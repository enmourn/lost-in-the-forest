export default class Alert {
  status: null | "about" | "intro" | "start" | "win" | "attempt1" | "attempt2" | "loss"
  ...
  render({ ctx, element }: { ctx: CanvasRenderingContext2D; element: HTMLCanvasElement }) {
    ...
    if (this.status === "about") {
      ...
      text = [
        ...
        "наглядно показать, что происходит с человеком,",
      ...
      text = [
        ...
        "стоило вам только зайти в лес, как корина для грибов",
        "начала стремительно наполняться...",
        "",
      ...
      ctx.fillRect(coord.x, coord.y, alertW, alertH)
      text = [
        "Вы потратили немало сил и вам захотелось ненадолго",
        "остановиться и отдохнуть. Небо, еще недавно озаренное",
        "солнцем, плотно затянуло тяжелыми дождевыми тучами.",
        "Вы почувствовали, как холодный ветер предательски",
        "пробивается сквозь одежду, вызывая прохладу и легкую",
        "дрожь в теле. Теплый шерстяной свитер, оставленный",
        "дома, вам бы сейчас очень пригодился... Чтобы не",
        "замерзнуть - нужно двигаться!",
        "",
        "Нажмите Enter чтобы продолжить",
      ]
